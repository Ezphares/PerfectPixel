...
namespace perfectpixel { namespace bedrock {
	...
	struct PpCrc
	{
		static constexpr unsigned int crc32(const char *str, unsigned int prev = 0xffffffff)
		{
			return PpCrc<size, idx + 1>::crc32(str, (prev >> 8) ^ crc_table[(prev ^ str[idx]) & 0xff]);
		}
	};

	template<int size, class dummy>
	struct PpCrc<size, size, dummy> {
		static constexpr unsigned int crc32(const char *str, unsigned int prev = 0xffffffff)
		{
...
#define PP_KEY_EQUAL(key, num) (static_cast<int32_t>(::perfectpixel::bedrock::crc32(key, std::strlen(key))) == num)
#else
