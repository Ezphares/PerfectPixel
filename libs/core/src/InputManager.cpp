    ...
    bedrock::KeyboardButton keyCode, bedrock::KeyEvent keyEvent)
{
    ...
    if (binding != m_buttonBindings.end())
    {
        ...
        bool target  = keyEvent == bedrock::KeyEvent::PP_KEYDOWN,
...
int32_t InputManager::registerButton(const std::string &name)
{
    ...
    int32_t id = static_cast<uint32_t>(m_buttons.size());
...
{
    // FIXME check already existing
    int32_t id = static_cast<uint32_t>(m_axes.size());
...
void InputManager::bindAxisToButtons(
