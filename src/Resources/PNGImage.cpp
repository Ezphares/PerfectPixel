...
namespace perfectpixel { namespace resources {
	...
	void PNGImage::FromSimpleBuffer(PNGImage *out_image, char *buffer, size_t bufferSize, bool bgr, bool vflip)
	{
		const size_t PNG_HEADER_SIZE = 8;
		if (bufferSize < PNG_HEADER_SIZE)
		{
			throw bedrock::PpException("Input buffer too small to contain PNG header");
		}
...
#pragma warning(pop)
		...
		out_image->m_channels = png_get_channels(readStruct.m_png, readStruct.m_pngInfo);
	...
	void PNGImage::ReadFromSimpleBuffer(png_structp png_ptr, png_bytep out_bytes, png_size_t count)
	{
		png_voidp io_ptr = png_get_io_ptr(png_ptr);
		if (io_ptr == nullptr)
		{
			throw bedrock::PpException("NULL passed to ReadFromSimpleBuffer from libpng");
		}
