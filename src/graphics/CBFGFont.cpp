...
namespace perfectpixel {
	namespace graphics {
			...
			: m_texture(Texture::PLACEHOLDER)
		{
			...
			m_uvStride = {
				static_cast<float>(m_header.m_cellWidth) / static_cast<float>(m_header.m_imageWidth),
				static_cast<float>(m_header.m_cellHeight) / static_cast<float>(m_header.m_imageHeight)
			};
		}
		...
		void CBFGFont::writeBuffer(bedrock::Vector3 position, float size, const std::string &text, std::vector<TextVertex> *out_vertices)
		{
			...
			for (char c : text)
			{
				int32_t row = (c - m_header.m_base) / (m_header.m_imageWidth / m_header.m_cellWidth);
				int32_t col = (c - m_header.m_base) % (m_header.m_imageWidth / m_header.m_cellWidth);
