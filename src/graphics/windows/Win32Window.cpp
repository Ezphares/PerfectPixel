...
void Win32Window::setSplash(const resources::Image &image)
{
    ...
    if (image.getChannels() < 4)
    {
        buffer = new unsigned char
            [image.getBufferSize() / image.getChannels() * 4];
        memset(buffer, 0, image.getBufferSize() / image.getChannels() * 4);

        for (unsigned i = 0; i < image.getBufferSize() / image.getChannels();
             ++i)
        {
            memcpy(
                buffer + i * 4,
                image.getBuffer() + i * image.getChannels(),
                image.getChannels());
        }
    }

    m_splash
        = CreateBitmap(image.getSize().m_x, image.getSize().m_y, 1, 32, buffer);
