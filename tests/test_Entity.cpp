...
GTEST_TEST(Entity, test_entity_create_get)
{
	...
	EXPECT_EQ(0u, entity_0_0.index);

	Entity entity_100_0 = entityCreate(100, 0);
	EXPECT_EQ(100u, entity_100_0.generation);
	EXPECT_EQ(0u, entity_100_0.index);

	Entity entity_0_25 = entityCreate(0, 25);
	EXPECT_EQ(0u, entity_0_25.generation);
...
GTEST_TEST(Entity, test_entity_overflow)
{
	...
	EXPECT_EQ(1u, entity_1_0.generation);
	EXPECT_EQ(0u, entity_1_0.index);

	uint32_t indexMax = (1 << INDEX_BITS) - 1;
	Entity entity_0_1 = entityCreate(0, indexMax + 2);
	EXPECT_EQ(0u, entity_0_1.generation);
	EXPECT_EQ(01, entity_0_1.index);
